...
import classes from "./EditAddresssModal.module.css";
...
export default function EditAddressModal({
  ...
  const [buildingNo, setBuildingNo] = useState(bNo);
  const [buildingNoErr, setBuildingNoErr] = useState("");
  const [landMark, setLandMark] = useState(lMark);
  const [landMarkErr, setLandMarkErr] = useState("");
  const [addressLabel, setAddressLabel] = useState(aLabel);
  const [addressLabelErr, setAddressLabelErr] = useState("");
  const [showOther, setShowOther] = useState(false);

  useEffect(() => {
    if (aLabel !== "Home" && aLabel !== "Work") {
      setShowOther(true);
    }
  }, [aLabel]);

  const { updateAddress, userDetails } = useFirebase();

  const { addresses } = userDetails;

  const isHomeAddressDisabled = addresses.find(
    (address) => address.addressLabel === "Home" && address.id !== id
  );
  const isWorkAddressDisabled = addresses.find(
    (address) => address.addressLabel === "Work" && address.id !== id
  );
  ...
  const handleSubmit = async (e) => {
    ...
    await updateAddress(id, houseNo, buildingNo, landMark, addressLabel);
    closeModal();
  };
  return (
